// TC05: Recommendation - Scenario that should favour Testim as recommended tool
const { test, expect } = require('@playwright/test');

test('TC05: Testim recommendation scenario', async ({ page }) => {
  ...
  console.log('\nStep 2: Answering questions to favor Testim...');
  
  // Answer questions in a way that favors Testim
  // Testim is typically favored for: low-code, AI-powered, visual testing, maintenance
  ...
  // Testim-favoring keywords
  const testimKeywords = ['low', 'code', 'no-code', 'ai', 'visual', 'self', 'healing', 'maintenance', 'easy', 'simple', 'automated', 'intelligent'];
  ...
  while (questionsAnswered < requiredQuestions) {
    ...
    // First, try to find options with Testim-favoring keywords
    for (const keyword of testimKeywords) {
      try {
  ...
  console.log('\nStep 5: Verifying Testim recommendation...');
  
  // Wait for page to load
  await page.waitForTimeout(2000);
  
  // Check for Testim in various ways
  const testimSelectors = [
    'text=/testim/i',
    'text=/testim.*recommended/i',
    'text=/recommended.*testim/i',
    'text=/testim.*best/i',
    'text=/best.*testim/i',
    '[class*="testim" i]',
    '[id*="testim" i]',
    '[data-testid*="testim" i]'
  ];
  
  let testimFound = false;
  let testimText = '';
  
  for (const selector of testimSelectors) {
    try {
      const element = page.locator(selector).first();
      if (await element.isVisible({ timeout: 2000 })) {
        testimText = await element.textContent().catch(() => '');
        console.log(`  ✓ Found Testim: "${testimText.trim()}"`);
        testimFound = true;
        break;
      }
    } catch (e) {
  ...
  if (pageText && pageText.toLowerCase().includes('testim')) {
    console.log('  ✓ Found "Testim" in page content');
    testimFound = true;
  }
  ...
  console.log(`  Testim found: ${testimFound}`);
  ...
  // 3. Testim is mentioned on the page
  expect(questionsAnswered >= 12 || isOnResultsPageFinal || testimFound).toBeTruthy();
  
  console.log('\n✓ Testim recommendation test completed successfully');
});
